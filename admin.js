...
if(trendEl){const days=[...Array(7)].map((_,i)=>{const d=new Date();d.setDate(d.getDate()-(6-i));return d.toLocaleDateString('zh-CN',{month:'numeric',day:'numeric'})});const counts=days.map(day=>allRecords.filter(r=>String(r.time||'').includes(day)).length);const max=Math.max(1,...counts);trendEl.innerHTML=days.map((day,i)=>`<div class="trend-day"><strong>${counts[i]}</strong><span class="trend-bar" style="height:${Math.max(4,counts[i]/max*96)}px"></span><small>${day}</small></div>`).join('')}
